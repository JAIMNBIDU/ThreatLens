    ...
    <div className="tl-tint" style={base}>
      ...
      {title === "ThreatFox" && (
        <>
          {data.listed ? (
            <>
              <div style={{ fontSize: 15, fontWeight: 700, fontFamily: DISPLAY, color: "var(--sev-critical)", lineHeight: 1.1 }}>
                {data.malware_printable || data.malware || "Confirmed malware"}
              </div>
              <div style={{ fontSize: 10, color: "var(--text-faint)", marginTop: 3 }}>
                {(data.threat_type || "").replace(/_/g, " ")} · confidence {data.confidence}%
              </div>
              {data.tags?.length > 0 && (
                <div style={{ marginTop: 6, display: "flex", flexWrap: "wrap", gap: 3 }}>
                  {data.tags.slice(0, 6).map((t, i) => (
                    <span key={i} style={{ fontSize: 9, color: "var(--text-dim)", background: "var(--tag-bg)",
                      padding: "1px 5px", borderRadius: 4, fontFamily: MONO }}>{t}</span>
                  ))}
                </div>
              )}
            </>
          ) : (
            <div style={{ fontSize: 11, color: "var(--sev-low)" }}>Not listed · no malware association</div>
          )}
        </>
      )}
      {title === "Shodan" && (
    ...
    <div onClick={onClick} className="tl-card tl-rise" style={{
      ...
      <div style={{ display: "flex", alignItems: "center", gap: 11, marginBottom: 8 }}>
        ...
        <div style={{ flex: 1, minWidth: 0 }}>
          ...
          <div style={{ fontSize: 10, color: "var(--text-faint)", marginTop: 3, display: "flex", gap: 8, alignItems: "center", flexWrap: "wrap" }}>
            ...
            {score.boosted && (
              <span title={score.boosted_by?.join(", ")} style={{ background: "var(--sev-critical-badge)", color: "var(--sev-critical)",
                padding: "1px 7px", borderRadius: 4, fontSize: 9, letterSpacing: "0.05em", fontWeight: 700, fontFamily: DISPLAY }}>
                ◆ INTEL-CONFIRMED
              </span>
            )}
          </div>
      ...
      {isExpanded && (
        <div className="tl-expand" style={{ borderTop: "1px solid var(--border)", paddingTop: 14, marginTop: 13 }}>
          ...
          {score.boosted && (
            <div className="tl-tint" style={{ background: "var(--sev-critical-bg)", border: "1px solid var(--sev-critical-edge)",
              borderRadius: 7, padding: "9px 12px", marginBottom: 14, fontSize: 11, color: "var(--sev-critical)", lineHeight: 1.5 }}>
              <span style={{ fontFamily: DISPLAY, fontWeight: 700, letterSpacing: "0.04em" }}>INTEL FLOOR APPLIED.</span>{" "}
              <span style={{ color: "var(--text-dim)" }}>
                Source scoring gave {score.base_raw}, but confirmed intel ({score.boosted_by?.join(", ")}) floors the score at {score.intel_score}. A known threat isn't downgraded by a cold reputation score.
              </span>
            </div>
          )}
            ...
            <div style={{ display: "flex", gap: 6 }}>
              {Object.entries(score.components).map(([src, val]) => (
                <div key={src} className="tl-tint" style={{ flex: 1, background: "var(--inset)", borderRadius: 6, padding: "6px 9px", border: "1px solid var(--border)" }}>
                  ...
                  <div style={{ fontSize: 9, color: "var(--text-ghost)" }}>×{({ virustotal: 0.45, abuseipdb: 0.35, shodan: 0.20 })[src] ?? ""}</div>
                </div>
              ))}
            </div>
          </div>
        ...
        {summary && (
          <div className="tl-rise" style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(102px, 1fr))", gap: 10, marginBottom: 22 }}>
            {[
              ...
              { label: "Confirmed", value: summary.intel_confirmed || 0,            color: "var(--sev-critical)" },
